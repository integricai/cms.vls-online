// Vercel serverless function — reads/writes vls-about-us.json in the GitHub Gist
...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    const file = data.files && data.files['vls-about-us.json'];
    let sections = [];
  ...
  if (req.method === 'POST') {
    ...
    const ghRes = await fetch(`https://api.github.com/gists/${gistId}`, {
      ...
      body: JSON.stringify({
        files: { 'vls-about-us.json': { content: JSON.stringify({ sections }, null, 2) } }
      })
    });
