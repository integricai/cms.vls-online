// Vercel serverless function — reads/writes vls-promotion-sections.json in the GitHub Gist
// GET  → returns { sections }
// POST → saves sections array
...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    const file = data.files && data.files['vls-promotion-sections.json'];
    let sections = [];
  ...
  if (req.method === 'POST') {
    ...
    const ghRes = await fetch(`https://api.github.com/gists/${gistId}`, {
      ...
      body: JSON.stringify({
        files: { 'vls-promotion-sections.json': { content: JSON.stringify({ sections }, null, 2) } }
      })
    });
