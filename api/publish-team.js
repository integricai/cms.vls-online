// Vercel serverless function — reads/writes vls-team.json in the GitHub Gist
// GET  → returns { cards }
// POST → saves cards array
...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    const file = data.files && data.files['vls-team.json'];
    let cards = [];
    if (file && file.content) {
      try { cards = JSON.parse(file.content).cards || []; } catch(e) {}
    }
    return res.status(200).json({ cards });
  }

  // ── POST: save team cards ───────────────────────────────────
  if (req.method === 'POST') {
    let body;
    try {
    ...
    const ghRes = await fetch(`https://api.github.com/gists/${gistId}`, {
      ...
      body: JSON.stringify({
        files: { 'vls-team.json': { content: JSON.stringify({ cards }, null, 2) } }
      })
    });
