// Vercel serverless function — reads/writes vls-header-config.json in the GitHub Gist
// GET  → returns { config }
...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    const file = data.files && data.files['vls-header-config.json'];
    let config = null;
  ...
  if (req.method === 'POST') {
    ...
    const ghRes = await fetch(`https://api.github.com/gists/${gistId}`, {
      ...
      body: JSON.stringify({
        files: { 'vls-header-config.json': { content: JSON.stringify({ config }, null, 2) } }
      })
    });
