// Vercel serverless function — reads/writes vls-banners.json in the GitHub Gist
// GET  → returns { banners }
// POST → saves banners array
...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    const file = data.files && data.files['vls-banners.json'];
    let banners = [];
    if (file && file.content) {
      try { banners = JSON.parse(file.content).banners || []; } catch(e) {}
    }
  ...
  if (req.method === 'POST') {
    ...
    const ghRes = await fetch(`https://api.github.com/gists/${gistId}`, {
      ...
      body: JSON.stringify({
        files: { 'vls-banners.json': { content: JSON.stringify({ banners }, null, 2) } }
      })
    });
