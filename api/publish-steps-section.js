// Vercel serverless function — reads/writes vls-steps-sections.json in the GitHub Gist
...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    const file = data.files && data.files['vls-steps-sections.json'];
    let sections = [];
  ...
  if (req.method === 'POST') {
    ...
    const ghRes = await fetch(`https://api.github.com/gists/${gistId}`, {
      ...
      body: JSON.stringify({
        files: { 'vls-steps-sections.json': { content: JSON.stringify({ sections }, null, 2) } }
      })
    });
