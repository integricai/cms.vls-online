...
export default function FeatureCardsV3Screen() {
  ...
  useEffect(() => {
    api.get<{ data: Fc3Content & { sections?: any[] } }>('/content/vls-feature-cards-3')
      .then(row => {
  ...
  function duplicateComponent() {
    setActiveId(null);
    setName(name ? `${name} (Copy)` : 'Feature Cards V3 Copy');
    setState(JSON.parse(JSON.stringify(state)) as Fc3State);
    setSaved(false);
  }
  ...
  async function save() {
    ...
    try {
      ...
      if (id) {
        ...
        if (!comps.find(c => c.id === id)) { id = `fc3-${Date.now().toString(36)}`; comps.push({ id, name, data: state }); }
      } else { id = `fc3-${Date.now().toString(36)}`; comps.push({ id, name, data: state }); }
    ...
    <div className="flex h-full">
      ...
      <div className="w-[480px] shrink-0 overflow-y-auto border-r border-slate-200 bg-white">
        <div className="sticky top-0 z-10 border-b border-slate-100 bg-white px-5 py-4">
          <h1 className="text-base font-bold text-slate-900">Feature Card v3</h1>
          <p className="text-xs text-slate-400 mt-0.5">Cards with coloured header, number badge, and tag rows</p>
        </div>
        ...
        <div className="px-5 py-4">
          ...
          <div className="rounded-lg border border-slate-200 bg-slate-50 p-3 mb-4">
            ...
            <Field label="Component name">
              <input className="input" value={name} placeholder="e.g. Program Overview Cards"
          ...
          <div className="grid grid-cols-2 gap-2">
            <Field label="Eyebrow text">
              <input className="input" value={state.eyebrow} placeholder="FEATURED PROGRAMS"
            ...
            <Field label="Heading text">
              <input className="input" value={state.headingText} placeholder="Our programs"
              ...
              <div key={ci} className="relative rounded border border-slate-200 bg-slate-50 p-3">
                ...
                <div className="grid grid-cols-3 gap-2">
                  ...
                  <Field label="Title" className="col-span-2">
                    <input className="input" value={card.title} placeholder="Card title"
                      onChange={e => updateCard(ci, { title: e.target.value })} />
                  </Field>
                </div>
                <Field label="Subtitle">
                  <input className="input" value={card.subtitle} placeholder="Short subtitle"
                ...
                <button onClick={() => addTag(ci)} className="btn-ghost text-xs w-full">+ Add tag</button>
              </div>
            ))}
          </div>
