...
export function generateBannerHtml(b: Banner, deadlineMs: number): string {
...
.vls-bnp-cta{display:none!important;}
  ...
  if (title.text) html += `<div style="font-family:'Poppins',sans-serif;line-height:1.3;${textStyle(title)}">${escapeHtml(title.text)}</div>`;
  if (subtitle.text) html += `<div style="font-family:'Poppins',sans-serif;opacity:0.8;margin-top:3px;${textStyle(subtitle)}">${escapeHtml(subtitle.text)}</div>`;
  html += `</div>`;
