...
function BannerForm({ banner: b, onChange }: { banner: Banner; onChange: (patch: Partial<Banner>) => void }) {
  ...
  return (
    <div className="space-y-0">
      ...
      <Field label="When timer expires">
        <select className="input" value={b.hideOnExpiry ? 'hide' : 'keep'}
          onChange={e => onChange({ hideOnExpiry: e.target.value === 'hide' })}>
          <option value="hide">Hide banner automatically</option>
          <option value="keep">Keep banner visible (no countdown)</option>
        </select>
      </Field>
...
function buildInjectCode(banner: Banner): string {
  const bid = banner.id;
  return `<script>
(function(){
    ...
    <div className="flex h-full">
      ...
      <div className="w-[420px] shrink-0 overflow-y-auto border-r border-slate-200 bg-white">
        <div className="sticky top-0 z-10 border-b border-slate-100 bg-white px-5 py-4">
          ...
          <p className="text-xs text-slate-400 mt-0.5">Sticky countdown banners — paste inject code once, publish updates live</p>
        </div>

        {/* ── Action buttons ── */}
        <div className="border-b border-slate-100 bg-white px-5 py-3 space-y-2">
          <div className="grid grid-cols-2 gap-2">
            <button
            ...
            >
              {publishing ? 'Publishing…' : published ? '✓ Published' : '🚀 Publish Banner'}
            </button>
          </div>
          {publishError && (
            <p className="text-xs text-red-600 bg-red-50 border border-red-200 rounded px-3 py-2">{publishError}</p>
          )}
          <div className="rounded bg-slate-50 border border-slate-200 px-3 py-2 space-y-1">
            <p className="text-[11px] text-slate-500">
              <span className="font-semibold">Save &amp; Generate HTML</span> — paste the code once into your Zenler site.
            </p>
            <p className="text-[11px] text-slate-500">
              <span className="font-semibold">Publish Banner</span> — after the code is pasted, use this to push content and timer changes live without re-pasting.
            </p>
          </div>
        </div>
          ...
          <div className="mb-4">
            ...
            {banners.length === 0 ? (
              ...
              <div className="space-y-1">
                {banners.map(b => (
                  <div key={b.id} onClick={() => selectBanner(b.id)}
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        {activeTab === 'preview' ? (
          ...
          <div className="relative flex-1 overflow-auto bg-slate-900 p-4">
            {injectCode && (
              <button
                onClick={() => navigator.clipboard.writeText(injectCode)}
