...
export function generateCoursePriceHtml(price: CoursePrice): string {
  ...
  const eyebrowSz  = fs(price.eyebrowSize,  8,  24, 11);
  const eyebrowWt  = fw(price.eyebrowWeight, 700);
  const titleSz    = fs(price.titleSize,    10,  36, 15);
  const titleWt    = fw(price.titleWeight,   700);
  const amountSz   = fs(price.amountSize,   20,  80, 46);
  const bodySz     = fs(price.bodySize,     10,  20, 13);
  const ctaSz      = fs(price.ctaSize,      10,  24, 14);
  const ctaWt      = fw(price.ctaWeight,    800);

  return `<style>
.vls-price-card{box-sizing:border-box;width:100%;max-width:300px;background:${price.bg || '#ffffff'};border:1px solid ${price.border || '#e5e7eb'};border-top:4px solid ${price.accent || '#204280'};border-radius:${radius}px;overflow:hidden;font-family:'${ff}',Arial,sans-serif;color:#111827;box-shadow:0 8px 24px rgba(15,23,42,.08);}
.vls-price-card *{box-sizing:border-box;}
.vls-price-head{padding:18px 20px 16px;border-bottom:1px solid #e5e7eb;}
.vls-price-eyebrow{display:flex;align-items:center;gap:7px;font-size:${eyebrowSz}px;font-weight:${eyebrowWt};letter-spacing:.08em;text-transform:uppercase;color:${price.accent || '#204280'};}
.vls-price-eyebrow:before{content:"";width:5px;height:5px;border-radius:999px;background:${price.accent || '#204280'};}
.vls-price-title{margin-top:6px;font-size:${titleSz}px;line-height:1.35;font-weight:${titleWt};color:#111827;}
...
.vls-price-amount{font-size:${amountSz}px;font-weight:800;line-height:1;}
...
.vls-price-inc li{display:grid;grid-template-columns:18px 1fr;gap:9px;align-items:start;font-size:${bodySz}px;line-height:1.35;color:#111827;}
...
.vls-price-cta{display:block;margin-top:30px;width:100%;border:1px solid #d1d5db;border-radius:7px;background:#ffffff;color:#111827;text-decoration:none;text-align:center;padding:10px 12px;font-size:${ctaSz}px;font-weight:${ctaWt};}
.vls-price-refund{margin:10px 0 0;text-align:center;color:#4b5563;font-size:11px;}
@media(max-width:640px){.vls-price-card{max-width:none}.vls-price-amount{font-size:${Math.max(20, amountSz - 4)}px}}
