import type { Pcv2State } from '../../types/cms';
...
function chipsHtml(chips: string, uid: string, chipStyle: string): string {
  return String(chips || '').split(/\n+/).map(c => c.trim()).filter(Boolean)
    .map(chip => `<span class="${uid}-chip" style="${chipStyle}">${escapeHtml(chip)}</span>`)
    .join('');
}

export function generateProgramCardsV2Html(data: Pcv2State): string {
