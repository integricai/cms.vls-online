...
import type { FcState, FcComponent, FcContent, FcCard, TextValue } from '../../types/cms';
...
export default function FeatureCardsScreen() {
  const [components, setComponents] = useState<FcComponent[]>([]);
  const [activeId, setActiveId]     = useState<string | null>(null);
  const [name, setName]             = useState('');
  const [state, setState]           = useState<FcState>(makeDefault());
  ...
  useEffect(() => {
    api.get<{ data: FcContent & { sections?: any[] } }>('/content/vls-feature-cards')
      .then(row => {
        const raw = row?.data as any;
        let comps: FcComponent[] = [];
        if (raw?.components) {
  ...
  const upd = useCallback((patch: Partial<FcState>) => { setState(prev => ({ ...prev, ...patch })); setSaved(false); }, []);
  ...
  function duplicateComponent() {
    setActiveId(null);
    setName(name ? `${name} (Copy)` : 'Feature Cards Copy');
    setState(JSON.parse(JSON.stringify(state)) as FcState);
    setSaved(false);
  }
  ...
  async function save() {
    ...
    try {
      ...
      if (id) {
        ...
        if (!comps.find(c => c.id === id)) { id = `fc-${Date.now().toString(36)}`; comps.push({ id, name, data: state }); }
      } else { id = `fc-${Date.now().toString(36)}`; comps.push({ id, name, data: state }); }
  ...
  function updateCard(i: number, patch: Partial<FcCard>) { const a = [...state.cards]; a[i] = { ...a[i], ...patch }; upd({ cards: a }); }
    ...
    <div className="flex h-full">
      ...
      <div className="w-[480px] shrink-0 overflow-y-auto border-r border-slate-200 bg-white">
        <div className="sticky top-0 z-10 border-b border-slate-100 bg-white px-5 py-4">
          <h1 className="text-base font-bold text-slate-900">Feature Cards</h1>
          <p className="text-xs text-slate-400 mt-0.5">3-column card grid with coloured top section</p>
        </div>
        ...
        <div className="px-5 py-4">
          ...
          <div className="rounded-lg border border-slate-200 bg-slate-50 p-3 mb-4">
            ...
            <Field label="Component name">
              <input className="input" value={name} placeholder="e.g. Home Feature Cards"
          ...
          <div className="grid grid-cols-2 gap-2">
            <Field label="Padding left (px)">
              <input type="number" className="input" min={0} max={200} value={state.padLeft}
                onChange={e => upd({ padLeft: Number(e.target.value) })} />
            </Field>
            <Field label="Padding right (px)">
              <input type="number" className="input" min={0} max={200} value={state.padRight}
                onChange={e => upd({ padRight: Number(e.target.value) })} />
            </Field>
          </div>

          {/* Section header */}
          <p className="section-label">Section Header</p>
          <RichTextField label="Eyebrow" value={asTV(state.eyebrow, 'featureEyebrow')} defaultKey="featureEyebrow"
            onChange={v => upd({ eyebrow: v })} />
          <RichTextField label="Title" value={asTV(state.title, 'featureTitle')} defaultKey="featureTitle"
          ...
          <div className="space-y-2 mb-2">
            {state.cards.map((card, i) => (
              <div key={i} className="relative rounded border border-slate-200 bg-slate-50 p-3">
                ...
                <ColorRow label="Top colour" value={card.color} onChange={v => updateCard(i, { color: v })} />
                <RichTextField label="Eyebrow badge" value={asTV(card.eyebrow, 'featureCardEyebrow')} defaultKey="featureCardEyebrow"
                  onChange={v => updateCard(i, { eyebrow: v })} />
                <RichTextField label="Card title" value={asTV(card.title, 'featureCardTitle')} defaultKey="featureCardTitle"
                  onChange={v => updateCard(i, { title: v })} />
                <RichTextField label="Subtitle" value={asTV(card.subtitle, 'featureCardSubtitle')} defaultKey="featureCardSubtitle"
                  onChange={v => updateCard(i, { subtitle: v })} />
                <RichTextField label="CTA text" value={asTV(card.ctaText, 'featureCardCta')} defaultKey="featureCardCta"
