...
function PanelEditor({ type }) {
    ...
    useEffect(() => {
        api.get(`/content/${config.key}`)
            .then(row => {
            const next = (row?.data?.sections || []).map(s => ({ ...blank, ...s, cards: (s.cards || []).map(card => ({ ...card, type: card.type || 'card' })) }));
            setItems(next);
            if (next[0]) {
                setActiveId(next[0].id);
                setName(next[0].name);
