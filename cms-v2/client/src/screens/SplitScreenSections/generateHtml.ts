...
function tv(value: TextValue | undefined, key: DefaultKey) {
...
function cardHtml(card: SplitSectionCard, mode: 'left' | 'right', imageBoxWidth: number, imageBoxHeight: number) {
  ...
  if (type === 'card-image') {
    ...
    let body = '';
    if (title.text) body += `<div style="font-family:'Poppins',sans-serif;margin:0 0 5px;${textStyle(title)}">${escapeHtml(title.text)}</div>`;
    if (descText) body += `<div style="font-family:'Poppins',sans-serif;margin:0${cta.text ? ' 0 8px' : ''};line-height:1.45;${textStyle(desc)}">${descText}</div>`;
    if (cta.text) body += `<div class="split-card-img-cta" style="display:flex;justify-content:flex-end;"><a href="${attr(card.ctaUrl || '#')}" style="display:inline-block;font-family:'Poppins',sans-serif;padding:8px 18px;background:${safeHex(card.ctaBg, defaults.ctaBg)};border-radius:6px;text-decoration:none;text-align:center;${textStyle(cta)}color:${safeHex(card.ctaColor, '#ffffff')};">${escapeHtml(cta.text)}</a></div>`;
    const spanStyle = mode === 'left' ? `grid-column:span ${card.halfWidth ? 1 : 2};` : '';
...
export function generatePanelHtml(section: SplitContentSection, mode: 'left' | 'right') {
  ...
  const sectionStyle = mode === 'left'
    ? ' style="padding:24px 20px;"'
    : ` style="padding:${clampInt(section.padTop, 20, 0, 240)}px ${clampInt(section.padRight, 20, 0, 240)}px ${clampInt(section.padBot, 20, 0, 240)}px ${clampInt(section.padLeft, 20, 0, 240)}px;"`;
...
<div class="${uid}"${sectionStyle}>
${eyebrow.text ? `  <div style="font-family:'Poppins',sans-serif;text-transform:uppercase;margin:0 0 ${mode === 'left' ? 10 : 16}px;${textStyle(eyebrow)}">${escapeHtml(eyebrow.text)}</div>\n` : ''}${heading.text ? `  <h2 style="font-family:'Poppins',sans-serif;margin:0 0 16px;line-height:1.2;${textStyle(heading)}">${escapeHtml(heading.text)}</h2>\n` : ''}${desc.text ? `  <div style="font-family:'Poppins',sans-serif;margin:0 0 24px;line-height:1.6;${textStyle(desc)}">${desc.text}</div>\n` : ''}${cards ? `  <div class="${uid}-cards">\n${cards}\n  </div>\n` : ''}</div>`;
}
...
function scrollAttrs(uid: string, value: string | undefined): string {
  const raw = String(value || '').trim();
  if (!raw) return '';
  const fn = `${uid}Scroll`;
...
export function generateLeftHeroHtml(data: PageLeftHeroState) {
...
`;
  ...
  if (labels.length) {
    ...
    labels.forEach((label, i) => {
      if (i > 0) out += `<span style="color:${safeHex(data.eyebrowTc, '#4a90d9')};margin:0 8px;font-family:'Poppins',sans-serif;font-size:11px;">&middot;</span>`;
  ...
  if (pathway.length) {
    ...
    pathway.forEach((item, i) => {
      if (i > 0) out += `    <span style="font-family:'Poppins',sans-serif;font-size:16px;color:${safeHex(data.arrowColor, '#4a90d9')};">-&gt;</span>\n`;
  ...
  if (trust.length) {
    ...
    trust.forEach((item, i) => {
      if (i > 0) out += `    <span style="display:inline-block;width:4px;height:4px;border-radius:50%;background:${safeHex(data.trustSep, '#4a90d9')};margin:0 10px;vertical-align:middle;flex-shrink:0;"></span>\n`;
