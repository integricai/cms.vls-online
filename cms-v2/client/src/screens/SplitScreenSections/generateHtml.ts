...
`;
  ...
  if (data.primaryCta || data.secondaryCta) {
    ...
    if (data.primaryCta) out += `    <a href="${attr(data.primaryCtaUrl || '#')}" style="display:inline-block;padding:13px 26px;background:${safeHex(data.primaryBg, '#204280')};color:${safeHex(data.primaryTc, '#ffffff')};font-family:'Poppins',sans-serif;font-size:14px;font-weight:600;border-radius:8px;text-decoration:none;">${escapeHtml(data.primaryCta)}</a>\n`;
    if (data.secondaryCta) out += `    <a href="${attr(data.secondaryCtaUrl || '#')}" style="display:inline-block;padding:13px 26px;border:2px solid ${safeHex(data.secondaryBorder, '#4a5568')};color:${safeHex(data.secondaryTc, '#ffffff')};font-family:'Poppins',sans-serif;font-size:14px;font-weight:600;border-radius:8px;text-decoration:none;background:transparent;">${escapeHtml(data.secondaryCta)}</a>\n`;
    out += '  </div>\n';
  }
