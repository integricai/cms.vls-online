...
<section class="${uid}">
  ...
  <div class="${uid}-wrap">
    ...
    <form class="${uid}-main" action="https://api.cms.vls-online.com/api/submit-report" method="post">
      ...
      <div class="${uid}-field"><label>Qualification</label><select name="qualification">${quals.map((q) => `<option>${escapeHtml(q)}</option>`).join('')}</select></div>
      <div class="${uid}-field"><label>Issue type</label><select name="issueType">${issueTypes.map((issue) => `<option>${escapeHtml(issue)}</option>`).join('')}</select></div>
...
export function generateReportTyHtml(config) {
  ...
  <div style="margin:4px 24px 20px;border:1px solid #e5e7eb;border-radius:10px;padding:18px 20px;background:#fff;">
    ...
    ${contacts.map((item) => `<p style="font-size:13px;color:#204280;margin:0 0 5px;">${escapeHtml(item)}</p>`).join('')}
  </div>
