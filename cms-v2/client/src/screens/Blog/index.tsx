...
function combinedCleanupHtml(): string {
  return `<script data-cfasync="false">(function(){function hide(el){if(!el)return;el.style.setProperty('display','none','important');el.style.setProperty('visibility','hidden','important');el.style.setProperty('height','0','important');el.style.setProperty('min-height','0','important');el.style.setProperty('max-height','0','important');el.style.setProperty('margin','0','important');el.style.setProperty('padding','0','important');el.style.setProperty('border','0','important');el.style.setProperty('overflow','hidden','important');el.style.setProperty('pointer-events','none','important');}function hasCmsBlog(){return !!document.querySelector('.vls-blog,.blog-content .vls-blog,#zen_blog_post .vls-blog');}function keep(el){if(!el||!el.matches)return false;if(el.matches('script,style,link,meta'))return true;if(el.matches('.vls-blog-header-generated,.vls-blog,.vlsft-generated,p.blog-content,.blog-content'))return true;if(el.querySelector&&el.querySelector('.vls-blog-header-generated,.vls-blog,.vlsft-generated'))return true;return false;}function cleanBlogChrome(){if(!hasCmsBlog())return;document.querySelectorAll('#zen_blog_post nav,#zen_blog_post>.blog-heading,#zen_blog_post>h1,#zen_blog_post>.zbv-blog-05-user,#zen_blog_post>img.zbv-blog-05-img,#zen_blog_post>p.zbv-blog-05-tag,#zen_blog_post>p.dynamic-text:not(.blog-content)').forEach(hide);document.querySelectorAll('.blog-content p.dynamic-text,#zen_blog_post>p.dynamic-text').forEach(function(p){if(p.matches('.blog-content'))return;if(p.querySelector('.vls-blog-header-generated,.vls-blog'))return;if(p.querySelector('style')&&!p.querySelector('.vls-blog')){hide(p);return;}if(p.closest('#zen_blog_post')&&p.matches('p.dynamic-text')&&!p.matches('.blog-content'))hide(p);});document.querySelectorAll('.blog-content p:empty,.blog-content p.zbv-blog-05-tag').forEach(hide);var holder=document.querySelector('#zen_blog_post,[data-zd="zen_blog_post"]');if(holder){Array.prototype.slice.call(holder.children).forEach(function(child){if(!keep(child))hide(child);});}}function clean(){document.querySelectorAll('.block.footer-style.parrot.zenstyle.bg-layer.padding-top-50.padding-bottom-50.center-center.footer-block,.footer-block[data-zen="zen_footer_dynamic"],[data-zen="zen_footer_dynamic"]').forEach(function(el){if(!el.querySelector('.vlsft-generated'))hide(el);});cleanBlogChrome();}if(document.readyState==='loading'){document.addEventListener('DOMContentLoaded',clean);}else{clean();}setTimeout(clean,300);setTimeout(clean,1000);setTimeout(clean,2500);})();<\/script>`;
}
