...
function buildZenNav(attempt){var zenUl=getZenlerMenuRoot();if(zenUl&&zenUl.querySelectorAll("li").length>0){bindZenlerMenuObserver();var items=readZenlerMenu();if(!items.length&&(attempt||0)<15){setTimeout(function(){buildZenNav((attempt||0)+1);},300);return;}if(countMenuChildren(items)===0&&hasMenuChildren(STATIC_FALLBACK_MENU)&&countMenuChildren(STATIC_FALLBACK_MENU)>0&&(attempt||0)<15){setTimeout(function(){buildZenNav((attempt||0)+1);},300);return;}items=addLoggedInSystemItems(items);renderNavItems(items);syncZenlerMenuVisibility();initOverflow();}else if((attempt||0)<15){setTimeout(function(){buildZenNav((attempt||0)+1);},300);}else if(STATIC_FALLBACK_MENU&&STATIC_FALLBACK_MENU.length){var items=addLoggedInSystemItems(STATIC_FALLBACK_MENU);renderNavItems(items);initOverflow();}}
