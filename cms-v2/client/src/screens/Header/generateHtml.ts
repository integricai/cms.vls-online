...
.${uid}-login-fallback{display:inline-flex;align-items:center;justify-content:center;color:${menuTextColor}!important;text-decoration:none!important;font-size:14px!important;font-weight:600!important;line-height:1.2!important;white-space:nowrap!important;}
...
function ensureLoginFallback(slot){if(!slot||window.innerWidth>768)return;var fallback=document.getElementById(P+"-login-fallback");if(fallback)return;fallback=document.createElement("a");fallback.id=P+"-login-fallback";fallback.className=P+"-login-fallback";fallback.href="/login";fallback.textContent="Login";fallback.style.setProperty("color",LOGIN_COLOR,"important");slot.appendChild(fallback);}
function moveLogin(){var slot=document.getElementById(window.innerWidth<=768?P+"-login-menu":P+"-login-brand");if(!slot)return false;var src=document.querySelector(".navbar-buttons.jqLoginLogout");if(!src){ensureLoginFallback(slot);return false;}var fallback=document.getElementById(P+"-login-fallback");if(fallback)fallback.remove();if(!slot.contains(src)){slot.appendChild(src);}forceLoginVisible(src);return true;}
...
var _lo=new MutationObserver(function(){moveLogin();});_lo.observe(document.body,{childList:true,subtree:true});setTimeout(function(){_lo.disconnect();},60000);
})();<\/script>`;
