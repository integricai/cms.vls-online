...
var USE_ZEN=true;
function getZenlerMenuRoot(){return document.querySelector("#vls-blog-zen-menu-source ul.dynamic_menu_texts")||document.querySelector(".block.parrot.zenstyle.headers ul.dynamic_menu_texts,.zbv-blog-nav ul.dynamic_menu_texts")||document.querySelector("ul.dynamic_menu_texts:not([data-vls-ignore-menu='1'])");}
function readZenlerMenu(){var items=[];var root=getZenlerMenuRoot();if(!root)return items;function linkItem(a){return{label:(a.textContent||"").trim(),url:a.getAttribute("href")||"#",newTab:a.getAttribute("target")==="_blank",children:[]};}function directLink(li){return Array.from(li.children).find(function(el){return el&&el.tagName==="A"&&el.classList.contains("dynamic-link");});}var lis=Array.from(root.children).filter(function(el){return el&&el.tagName==="LI";});lis.forEach(function(li){var action=li.getAttribute("data-action");var item={label:"",url:"#",newTab:false,children:[]};if(action==="link"){var a=li.querySelector("a.dynamic-link");if(!a)return;item=linkItem(a);}else if(action==="dropdown"){var tog=li.querySelector("a.dropdown-toggle");if(!tog)return;var clone=tog.cloneNode(true);var caret=clone.querySelector(".caret");if(caret)caret.remove();item.label=clone.textContent.trim();li.querySelectorAll("ul.dropdown-menu>li").forEach(function(sub){var sa=sub.querySelector("a.dynamic-link");if(!sa)return;var child=linkItem(sa);if(child.label)item.children.push(child);});}else{var plain=directLink(li)||li.querySelector("a.dynamic-link");if(!plain)return;item=linkItem(plain);}if(item.label)items.push(item);});return items;}
...
function buildZenNav(attempt){var zenUl=getZenlerMenuRoot();if(zenUl&&zenUl.querySelectorAll("li").length>0){var items=readZenlerMenu();if(!items.length&&(attempt||0)<15){setTimeout(function(){buildZenNav((attempt||0)+1);},300);return;}var ul=document.getElementById(P+"-ul");var moreItem=document.getElementById(P+"-more");if(!ul)return;Array.from(ul.children).forEach(function(li){if(li!==moreItem)ul.removeChild(li);});items.forEach(function(item){ul.insertBefore(makeNavLi(item,1),moreItem);});initOverflow();}else if((attempt||0)<15){setTimeout(function(){buildZenNav((attempt||0)+1);},300);}}
` : `var USE_ZEN=false;\n`;
