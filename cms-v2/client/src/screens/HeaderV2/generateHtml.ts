...
export function generateHeaderV2Html(cfg: HeaderV2Config): string {
  ...
  const brandInner = useV2BrandMark(cfg.logoUrl)
...
.vlsh2-logo{width:38px;height:38px;border-radius:10px;object-fit:cover;display:block;flex-shrink:0;}
.vlsh2-mark{width:38px;height:38px;border-radius:10px;background:#1e50c8;color:#fff;font-weight:700;font-size:19px;line-height:1;display:grid;place-items:center;box-shadow:0 4px 12px rgba(30,80,200,.3);flex-shrink:0;text-transform:none;}
...
function useMark(u){u=String(u||"").trim();return !u||/vertex-learning-solutions-01|vertex-logo-mark|69e4eae99c751|favicon\\.png/i.test(u);}
function setBrandMark(brand,url,alt){if(!brand)return;var mark=brand.querySelector(".vlsh2-mark");var logo=brand.querySelector(".vlsh2-logo");if(useMark(url)){if(logo)logo.remove();if(!mark){mark=document.createElement("span");mark.className="vlsh2-mark";mark.setAttribute("aria-hidden","true");mark.textContent="v";brand.insertBefore(mark,brand.firstChild);}}else if(url){if(mark)mark.remove();if(!logo){logo=document.createElement("img");logo.className="vlsh2-logo";logo.alt=alt||"Vertex Learning Solutions";brand.insertBefore(logo,brand.firstChild);}logo.src=url;if(alt)logo.alt=alt;}}
