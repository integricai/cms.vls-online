...
function slug(s){return String(s||"book").toLowerCase().replace(/&/g,"and").replace(/[^a-z0-9]+/g,"_").replace(/^_+|_+$/g,"").slice(0,150)||"book";}
function checkoutUrl(b){var raw=String(b.stripeUrl||b.sourceUrl||"#");if(!b.id||raw==="#")return raw;var ref=("book_"+b.id+"_"+slug(b.title)).slice(0,200);try{var u=new URL(raw,window.location.href);u.searchParams.set("client_reference_id",ref);return u.toString();}catch(e){return raw+(raw.indexOf("?")>=0?"&":"?")+"client_reference_id="+encodeURIComponent(ref);}}
