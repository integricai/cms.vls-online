...
function buildFooterMarkup(data, includeToggleScript) {
    const P = 'vlsft';
    const BG = '#0f2155';
    ...
    let html = `<footer class="${P}-generated" data-vls-footer="1" style="background:${BG};color:#fff;font-family:Poppins,sans-serif;">`;
...
function buildFooterUpdaterScript(publicFooterUrl) {
...
if(!root||!root.matches||!root.matches('[data-vls-footer="1"]'))root=document.querySelector('[data-vls-footer="1"]');
...
fetch(ENDPOINT+(ENDPOINT.indexOf('?')>-1?'&':'?')+'t='+Date.now()).then(function(r){return r.ok?r.json():null;}).then(function(payload){var data=payload&&(payload.footer||payload);if(data&&data.sections){root.outerHTML=render(data);}}).catch(function(){});
})();<\/script>`;
}
export function generateFooterHtml(data, publicFooterUrl) {
    return buildFooterMarkup(data, true) + buildFooterUpdaterScript(publicFooterUrl);
}
