...
export default function FeatureCardsV2Screen() {
    ...
    useEffect(() => {
        api.get('/content/vls-feature-cards-2')
            .then(row => {
            const raw = row?.data;
            let comps = [];
            if (raw?.components) {
    ...
    return (_jsxs("div", { className: "flex h-full", children: [_jsxs("div", { className: "w-[480px] shrink-0 overflow-y-auto border-r border-slate-200 bg-white", children: [_jsxs("div", { className: "sticky top-0 z-10 border-b border-slate-100 bg-white px-5 py-4", children: [_jsx("h1", { className: "text-base font-bold text-slate-900", children: "Feature Card v2" }), _jsx("p", { className: "text-xs text-slate-400 mt-0.5", children: "Centered grid with dotted dividers, uppercase titles & bordered CTAs" })] }), _jsxs("div", { className: "border-b border-slate-100 bg-white px-5 py-3 flex gap-2", children: [_jsx("button", { onClick: save, disabled: saving, className: "btn-primary flex-1 justify-center", children: saving ? 'Saving…' : saved ? '✓ Saved' : '💾 Save' }), _jsx("button", { onClick: () => { setPreviewHtml(wrapGeneratedHtml('Feature Cards V2', generateFeatureCardsV2Html(state))); setActiveTab('preview'); }, className: "btn-success flex-1 justify-center", children: "\u26A1 Generate HTML" })] }), _jsxs("div", { className: "px-5 py-4", children: [_jsxs("div", { className: "rounded-lg border border-slate-200 bg-slate-50 p-3 mb-4", children: [_jsx("p", { className: "section-label mt-0", children: "Saved Components" }), _jsxs("div", { className: "flex gap-2 mb-3", children: [_jsxs("select", { className: "input flex-1", value: activeId || '', onChange: e => loadComponent(e.target.value), children: [_jsx("option", { value: "", children: "\u2014 select to load \u2014" }), components.map(c => _jsx("option", { value: c.id, children: c.name }, c.id))] }), _jsx("button", { onClick: newComponent, className: "btn-ghost text-xs px-3", children: "+ New" }), activeId && _jsx("button", { onClick: deleteComponent, className: "btn-danger text-xs px-3", children: "Delete" })] }), _jsx(Field, { label: "Component name", children: _jsx("input", { className: "input", value: name, placeholder: "e.g. Course Features", onChange: e => setName(e.target.value) }) })] }), _jsx("p", { className: "section-label", children: "Layout" }), _jsxs("div", { className: "grid grid-cols-2 gap-2", children: [_jsx(ColorRow, { label: "Background", value: state.bg, onChange: v => upd({ bg: v }) }), _jsx(ColorRow, { label: "Separator colour", value: state.sepColor, onChange: v => upd({ sepColor: v }) }), _jsx(Field, { label: "Padding top (px)", children: _jsx("input", { type: "number", className: "input", min: 0, max: 200, value: state.padTop, onChange: e => upd({ padTop: Number(e.target.value) }) }) }), _jsx(Field, { label: "Padding bottom (px)", children: _jsx("input", { type: "number", className: "input", min: 0, max: 200, value: state.padBottom, onChange: e => upd({ padBottom: Number(e.target.value) }) }) }), _jsx(Field, { label: "Padding left (px)", children: _jsx("input", { type: "number", className: "input", min: 0, max: 200, value: state.padLeft, onChange: e => upd({ padLeft: Number(e.target.value) }) }) }), _jsx(Field, { label: "Padding right (px)", children: _jsx("input", { type: "number", className: "input", min: 0, max: 200, value: state.padRight, onChange: e => upd({ padRight: Number(e.target.value) }) }) }), _jsx(Field, { label: "Columns", children: _jsxs("select", { className: "input", value: state.cols, onChange: e => upd({ cols: Number(e.target.value) }), children: [_jsx("option", { value: 2, children: "2 columns" }), _jsx("option", { value: 3, children: "3 columns" }), _jsx("option", { value: 4, children: "4 columns" })] }) })] }), _jsx("p", { className: "section-label", children: "Cards" }), _jsx("div", { className: "space-y-2 mb-2", children: state.cards.map((card, i) => (_jsxs("div", { className: "relative rounded border border-slate-200 bg-slate-50 p-3", children: [_jsx("button", { onClick: () => removeCard(i), className: "btn-danger absolute right-2 top-2", children: "\u2715" }), _jsx(ColorRow, { label: "Accent line colour", value: card.lineColor, onChange: v => updateCard(i, { lineColor: v }) }), _jsx(RichTextField, { label: "Title", value: asTV(card.title, 'fc2Title'), defaultKey: "fc2Title", onChange: v => updateCard(i, { title: v }) }), _jsx(RichTextField, { label: "Description", value: asTV(card.desc, 'fc2Desc'), defaultKey: "fc2Desc", onChange: v => updateCard(i, { desc: v }), multiline: true }), _jsx(RichTextField, { label: "CTA text", value: asTV(card.ctaText, 'fc2Cta'), defaultKey: "fc2Cta", onChange: v => updateCard(i, { ctaText: v }) }), _jsx(Field, { label: "CTA URL", children: _jsx("input", { className: "input", value: card.ctaUrl, placeholder: "https://\u2026", onChange: e => updateCard(i, { ctaUrl: e.target.value }) }) })] }, i))) }), _jsx("button", { onClick: addCard, className: "btn-ghost text-xs w-full mb-4", children: "+ Add card" })] })] }), _jsxs("div", { className: "flex flex-1 flex-col overflow-hidden", children: [_jsx("div", { className: "flex border-b border-slate-200 bg-white px-4", children: ['preview', 'html'].map(tab => (_jsx("button", { onClick: () => setActiveTab(tab), className: `px-4 py-3 text-sm font-medium transition border-b-2 -mb-px ${activeTab === tab ? 'border-brand text-brand' : 'border-transparent text-slate-400 hover:text-slate-700'}`, children: tab === 'html' ? 'HTML' : 'Preview' }, tab))) }), activeTab === 'preview' ? (_jsx("iframe", { srcDoc: previewHtml
