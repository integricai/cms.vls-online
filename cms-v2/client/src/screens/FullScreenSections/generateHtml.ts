...
export function generateTestimonialsHtml(d: TestimonialsState, componentId = ''): string {
  ...
  L.push(`.${id}-country{display:flex;align-items:center;gap:4px;color:${e(d.dateColor)};font-size:var(--tst-date-sz);line-height:1;white-space:nowrap;}`);
  L.push(`.${id}-country svg{width:12px;height:12px;flex-shrink:0;}.${id}-country img{flex-shrink:0;border-radius:2px;}`);
