...
function tstQuoteHtml(value: string, charLimit = 250, wordLimit = 10000): string {
  ...
  return excerpt.truncated ? `${e(excerpt.text)}... <span class="__TST_READMORE__">Read More on Trustpilot</span>` : excerpt.html;
}
...
export function generateTestimonialsHtml(d: TestimonialsState, componentId = ''): string {
  ...
  cards.forEach(card => {
    ...
    L.push(`          <div class="${id}-quote">${tstQuoteHtml(card.quote, quoteCharLimit, quoteWordLimit).replace(/__TST_READMORE__/g, `${id}-readmore`)}</div>`);
