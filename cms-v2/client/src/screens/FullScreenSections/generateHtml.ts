...
function tstQuoteExcerpt(value: string): { text: string; html: string; truncated: boolean } {
...
function tstQuoteHtml(value: string): string {
  const excerpt = tstQuoteExcerpt(value);
  return excerpt.truncated ? `${e(excerpt.text)}...` : excerpt.html;
}

function tstReadMore(value: string, id: string): string {
  return tstQuoteExcerpt(value).truncated ? `<span class="${id}-readmore">Read More on Trustpilot</span>` : '';
}
...
export function generateTestimonialsHtml(d: TestimonialsState, componentId = ''): string {
  ...
  cards.forEach(card => {
    ...
    L.push(`          <div class="${id}-quote">${tstQuoteHtml(card.quote)}</div>`);
    L.push(`          ${tstReadMore(card.quote, id)}`);
