...
function tstQuoteReadMore(value: string, charLimit = 250, wordLimit = 10000): string {
  return tstQuoteExcerpt(value, charLimit, wordLimit).truncated ? '<span class="__TST_READMORE__">Read More on Trustpilot</span>' : '';
}
...
export function generateTestimonialsHtml(d: TestimonialsState, componentId = ''): string {
  ...
  cards.forEach(card => {
    ...
    const readMore = tstQuoteReadMore(card.quote, quoteCharLimit, quoteWordLimit).replace(/__TST_READMORE__/g, `${id}-readmore`);
