...
function tstQuoteExcerpt(value: string, charLimit = 250, wordLimit = 50): { text: string; html: string; truncated: boolean } {
...
function tstQuoteHtml(value: string, charLimit = 250, wordLimit = 50): string {
  const excerpt = tstQuoteExcerpt(value, charLimit, wordLimit);
  return excerpt.truncated ? `${e(excerpt.text)}...` : excerpt.html;
}

function tstReadMore(value: string, id: string, charLimit = 250, wordLimit = 50): string {
  return tstQuoteExcerpt(value, charLimit, wordLimit).truncated ? `<span class="${id}-readmore">Read More on Trustpilot</span>` : '';
}
...
export function generateTestimonialsHtml(d: TestimonialsState, componentId = ''): string {
  ...
  const quoteCharLimit = Math.max(70, Math.min(250, Math.round((cardHeight - 210) * 1.2)));
  const quoteWordLimit = Math.max(14, Math.min(50, Math.floor(quoteCharLimit / 5)));
  ...
  L.push(`.${id}-readmore{display:inline-block;flex:0 0 auto;margin:0 0 12px;color:${e(d.accentColor)};font-size:var(--tst-quote-sz);font-weight:800;line-height:1.35;text-decoration:underline;text-underline-offset:2px;}`);
  ...
  cards.forEach(card => {
    ...
    L.push(`          ${tstReadMore(card.quote, id, quoteCharLimit, quoteWordLimit)}`);
