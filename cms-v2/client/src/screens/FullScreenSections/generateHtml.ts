...
export function generateTestimonialsHtml(d: TestimonialsState, componentId = ''): string {
  ...
  L.push(`.${id}-summary-stars{display:flex;gap:2px;margin:0 0 8px;}`);
  L.push(`.${id}-summary-star{width:30px;height:30px;display:flex;align-items:center;justify-content:center;background:#00b67a;color:#fff;font-size:22px;font-weight:700;line-height:1;}`);
  L.push(`.${id}-summary-copy{font-size:14px;line-height:1.25;color:#30353e;margin:0;}`);
  ...
  L.push(`      <div class="${id}-summary" aria-hidden="true"><div class="${id}-summary-title">Excellent</div><div class="${id}-summary-stars"><span class="${id}-summary-star">★</span><span class="${id}-summary-star">★</span><span class="${id}-summary-star">★</span><span class="${id}-summary-star">★</span><span class="${id}-summary-star">★</span></div><p class="${id}-summary-copy">Based on <span>student reviews</span></p></div>`);
