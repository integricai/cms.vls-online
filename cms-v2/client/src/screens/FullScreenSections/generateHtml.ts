...
function tstCountryFlag(code: string): string {
  const lc = code.toLowerCase().replace(/[^a-z]/g, '');
  if (lc.length !== 2) return e(code);
  return `<img src="https://flagcdn.com/w20/${lc}.png" srcset="https://flagcdn.com/w40/${lc}.png 2x" width="20" height="15" alt="${e(code)}" style="border-radius:2px;vertical-align:middle;flex-shrink:0;">`;
}
...
export function generateTestimonialsHtml(d: TestimonialsState, componentId = ''): string {
  const id = uid();
  const cards = (d.cards || []).filter(card => card.enabled !== false && (card.quote || card.name || card.title || card.dateLabel || card.country));
  const url = (d.url || '#').trim() || '#';
  const gap = Math.max(8, Number(d.cardGap) || 18);
  const configuredCardHeight = Number(d.cardHeight);
  const cardHeight = configuredCardHeight > 220 ? 140 : Math.max(120, Math.min(220, configuredCardHeight || 140));
  const mobileCardHeight = Math.max(160, Math.min(260, cardHeight + 48));
  ...
  const reviewsText = (d.reviewsText || '').trim() || 'Based on reviews';
  const tpUrl = (d.trustpilotUrl || '').trim() || '#';
  ...
  L.push(`.${id}-date{display:inline;font-size:12px;font-weight:400;line-height:1.2;color:#6b6f76;margin:0;}`);
  L.push(`.${id}-date:before{content:", ";}.${id}-date svg{display:none;}`);
  L.push(`.${id}-country{display:none;}`);
  L.push(`.${id}-controls{height:0;overflow:hidden;margin:0;}`);
  L.push(`.${id}-btn{width:24px;height:24px;border-radius:999px;border:1px solid #9ba6b1;background:transparent;color:#1f2933;display:flex;align-items:center;justify-content:center;cursor:pointer;box-shadow:none;font-size:20px;line-height:1;padding:0;}`);
  L.push(`.${id}-btn:hover{background:#fff;}`);
  L.push(`.${id}-dots{display:none;}`);
  L.push(`.${id}-dot{display:none;}`);
  L.push(`@media(max-width:900px){.${id}-viewport{grid-template-columns:170px 28px minmax(0,1fr) 28px;}.${id}-card{flex-basis:calc((100% - ${gap}px) / 2);}}`);
  L.push(`@media(max-width:640px){.${id}-outer{padding:24px 18px!important;}.${id}-viewport{grid-template-columns:28px minmax(0,1fr) 28px;}.${id}-summary{display:none;}.${id}-clip{grid-column:2;}.${id}-card{flex-basis:100%;height:${mobileCardHeight}px;max-height:${mobileCardHeight}px;}.${id}-title{font-size:22px;}.${id}-sub{font-size:16px;}}`);
  L.push('</style>');
  ...
  L.push(`      <div class="${id}-summary"><a class="${id}-summary-link" href="${e(tpUrl)}" target="_blank" rel="nofollow noopener" aria-label="Read our Trustpilot reviews"><div class="${id}-summary-title">Excellent</div><div class="${id}-summary-stars"><span class="${id}-summary-star">★</span><span class="${id}-summary-star">★</span><span class="${id}-summary-star">★</span><span class="${id}-summary-star">★</span><span class="${id}-summary-star">★</span></div><p class="${id}-summary-reviews">${e(reviewsText)}</p><div class="${id}-trustpilot"><span class="${id}-trustpilot-star">★</span><span class="${id}-trustpilot-name">Trustpilot</span></div></a></div>`);
