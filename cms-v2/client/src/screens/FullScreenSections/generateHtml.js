...
// ── DCS (Two Column v1) ────────────────────────────────────────────────────────
export function generateDcsHtml(d) {
    const id = uid();
    ...
    if (lLabel.text)
        L.push(`    <div style="font-family:Poppins,sans-serif;${textStyle(lLabel)};margin-bottom:12px;text-transform:uppercase;">${e(lLabel.text)}</div>`);
    ...
    for (const p of d.leftParas) {
        ...
        if (para.text)
            L.push(`    <p style="font-family:Poppins,sans-serif;${textStyle(para)};margin:0 0 14px;line-height:1.7;">${e(para.text)}</p>`);
    }
    L.push(`  </div>`);
    // Right
    ...
    if (rLabel.text)
        L.push(`    <div style="font-family:Poppins,sans-serif;${textStyle(rLabel)};margin-bottom:20px;text-transform:uppercase;">${e(rLabel.text)}</div>`);
...
// ── DCS2 (Two Column v2) ───────────────────────────────────────────────────────
export function generateDcs2Html(d) {
    const id = uid();
    ...
    if (d.leftBullets?.length) {
        ...
        for (const b of d.leftBullets) {
            ...
            if (bt.text)
                L.push(`      <li style="font-family:Poppins,sans-serif;${textStyle(bt)};padding:6px 0;display:flex;align-items:center;gap:10px;"><span style="width:6px;height:6px;border-radius:50%;background:${e(d.bulletColor)};flex-shrink:0;display:inline-block;"></span>${e(bt.text)}</li>`);
        }
        L.push(`    </ul>`);
    }
    L.push(`  </div>`);
    ...
    for (const st of d.stats) {
        ...
        if (sl.text)
            L.push(`        <div style="font-family:Poppins,sans-serif;${textStyle(sl)};margin-top:4px;">${e(sl.text)}</div>`);
        L.push(`      </div>`);
    }
    L.push(`    </div>`);
    if (d.quoteShow) {
        ...
        if (qt.text)
            L.push(`      <p style="font-family:Poppins,sans-serif;${textStyle(qt)};margin:0 0 8px;line-height:1.6;">${e(qt.text)}</p>`);
        if (qa.text)
            L.push(`      <p style="font-family:Poppins,sans-serif;${textStyle(qa)};margin:0;">${e(qa.text)}</p>`);
        L.push(`    </div>`);
    }
    L.push(`  </div>`);
    L.push(`</div>`);
...
// ── DCS3 (Two Column v3) ───────────────────────────────────────────────────────
export function generateDcs3Html(d) {
    const id = uid();
    ...
    if (lPara.text)
        L.push(`    <p style="font-family:Poppins,sans-serif;${textStyle(lPara)};margin:0 0 24px;line-height:1.7;">${e(lPara.text)}</p>`);
    ...
    if (rLabel.text)
        L.push(`    <div style="font-family:Poppins,sans-serif;${textStyle(rLabel)};margin-bottom:12px;text-transform:uppercase;">${e(rLabel.text)}</div>`);
    L.push(`    <h3 style="font-family:Poppins,sans-serif;${textStyle(rTitle)};margin:0 0 16px;">${e(rTitle.text)}</h3>`);
    if (rPara.text)
        L.push(`    <p style="font-family:Poppins,sans-serif;${textStyle(rPara)};margin:0 0 24px;line-height:1.7;">${e(rPara.text)}</p>`);
    if (d.tags.length) {
...
export function generateReachHtml(d) {
    const id = uid();
    ...
    if (label.text)
        L.push(`      <div style="font-family:Poppins,sans-serif;${textStyle(label)};text-transform:uppercase;letter-spacing:0.12em;margin-bottom:12px;">${e(label.text)}</div>`);
    L.push(`      <h2 style="font-family:Poppins,sans-serif;${textStyle(title)};margin:0 0 16px;">${e(title.text)}</h2>`);
    if (para.text)
        L.push(`      <p style="font-family:Poppins,sans-serif;${textStyle(para)};line-height:1.7;margin:0;">${e(para.text)}</p>`);
    L.push(`    </div>`);
    ...
    if (d.stats.length) {
        ...
        for (const st of d.stats) {
            ...
            if (sl.text)
                L.push(`          <div style="font-family:Poppins,sans-serif;${textStyle(sl)};margin-top:6px;text-transform:uppercase;letter-spacing:0.08em;">${e(sl.text)}</div>`);
            L.push(`        </div>`);
        }
        L.push(`      </div>`);
        L.push(`    </div>`);
...
export function generatePhbHtml(d) {
    const L = [];
    ...
    if (d.eyebrow) {
        ...
        if (d.showDot)
            L.push(`        <span style="color:${e(d.dotColor)};">●</span>`);
        L.push(`        ${e(d.eyebrow)}`);
        L.push(`      </div>`);
    }
    ...
    if (d.bullets.length) {
        ...
        d.bullets.forEach((b, i) => {
            if (i > 0)
                L.push(`        <span style="color:${e(d.sepColor)};font-size:13px;">·</span>`);
    ...
    if (d.showBadge) {
        ...
        if (d.badgeEyebrow)
            L.push(`      <div style="font-family:Poppins,sans-serif;font-size:11px;font-weight:600;color:${e(d.badgeEyebrowTc)};letter-spacing:0.1em;text-transform:uppercase;margin-bottom:8px;">${e(d.badgeEyebrow)}</div>`);
        L.push(`      <div style="font-family:Poppins,sans-serif;${textStyle(badgeMain)};line-height:1;">${e(badgeMain.text)}</div>`);
        if (d.badgeSub)
            L.push(`      <div style="font-family:Poppins,sans-serif;font-size:13px;color:${e(d.badgeSubTc)};margin-top:8px;">${e(d.badgeSub)}</div>`);
        L.push(`    </div>`);
    }
    L.push(`  </div>`);
    L.push(`</div>`);
...
export function generatePhv2Html(d) {
    const id = uid();
    const L = [];
    ...
    if (heading.text)
        L.push(`      ${e(heading.text)}`);
    if (d.headingAccent)
        L.push(`      <span style="color:${e(d.headingAccentColor)};">${e(d.headingAccent)}</span>`);
    if (d.headingPost)
        L.push(`      ${e(d.headingPost)}`);
    L.push(`    </h1>`);
    if (desc.text) {
    ...
    if (d.trustItems.length) {
        ...
        d.trustItems.forEach((item, i) => {
            if (i > 0)
                L.push(`      <span style="color:${e(d.trustDot)};font-size:12px;">·</span>`);
            if (item.icon)
                L.push(`      <span>${e(item.icon)}</span>`);
...
export function generatePhv3Html(d) {
    const id = uid();
    const heading = n(d.heading, 'phv2Heading');
    const desc = n(d.desc, 'phv2Desc');
    ...
    if (d.breadcrumb)
        L.push(`      <div style="font-family:Poppins,sans-serif;font-size:12px;color:rgba(255,255,255,.55);margin-bottom:18px;">${e(d.breadcrumb)}</div>`);
    if (d.eyebrowLabels.length) {
    ...
    if (desc.text)
        L.push(`        <div class="${id}-desc" style="font-family:Poppins,sans-serif;${textStyle(desc)};line-height:1.7;color:${desc.color || '#c8e1f7'};">${e(desc.text)}</div>`);
    if (d.features.length) {
    ...
    if (d.primaryText)
        L.push(`          <a href="${e(d.primaryUrl || '#')}" style="display:inline-flex;align-items:center;justify-content:center;min-width:148px;min-height:50px;border-radius:9px;background:${e(d.primaryBg)};color:${e(d.primaryTc)};font-family:Poppins,sans-serif;font-size:14px;font-weight:800;text-decoration:none;">${e(d.primaryText)}</a>`);
    if (d.secondaryText)
        L.push(`          <a href="${e(d.secondaryUrl || '#')}" style="display:inline-flex;align-items:center;justify-content:center;min-width:164px;min-height:50px;border-radius:9px;background:${e(d.secondaryBg)};border:1px solid ${e(d.secondaryBorder)};color:${e(d.secondaryTc)};font-family:Poppins,sans-serif;font-size:14px;font-weight:700;text-decoration:none;">${e(d.secondaryText)}</a>`);
    L.push(`        </div>`);
    L.push(`      </div>`);
    L.push(`    </div>`);
    ...
    if (d.cardLabel)
        L.push(`          <span style="position:relative;z-index:1;display:inline-flex;border-radius:999px;background:rgba(255,255,255,.14);border:1px solid rgba(255,255,255,.26);padding:6px 13px;font-size:11px;font-weight:800;color:#ffffff;">${e(d.cardLabel)}</span>`);
    ...
    if (d.primaryText)
        L.push(`          <a href="${e(d.primaryUrl || '#')}" style="display:flex;align-items:center;justify-content:center;min-height:52px;border-radius:9px;background:${e(d.cardButtonBg)};color:#ffffff;font-size:14px;font-weight:800;text-decoration:none;margin-bottom:12px;">${e(d.primaryText)}</a>`);
    if (d.sampleText)
        L.push(`          <a href="${e(d.sampleUrl || '#')}" style="display:flex;align-items:center;justify-content:center;min-height:46px;border-radius:9px;background:#f5f7fb;border:1px solid #d9e1ec;color:#2168ad;font-size:13px;font-weight:800;text-decoration:none;margin-bottom:20px;">${e(d.sampleText)}</a>`);
    L.push(`          <div style="height:1px;background:#e7edf5;margin-bottom:16px;"></div>`);
    if (d.includesTitle)
        L.push(`          <div style="font-size:12px;font-weight:800;color:#9299a4;letter-spacing:.08em;margin-bottom:12px;">${e(d.includesTitle)}</div>`);
    ...
    if (d.refundText)
        L.push(`          <div style="margin-top:18px;border-radius:10px;background:#f6f7fb;padding:13px 14px;font-size:11px;line-height:1.55;color:#6d7480;">${e(d.refundText)}</div>`);
    L.push(`        </div>`);
    L.push(`      </aside>`);
    L.push(`    </div>`);
    L.push(`  </div>`);
...
export function generateBmsHtml(d) {
    const id = uid();
    ...
    if (d.checks.length) {
        ...
        for (const ch of d.checks) {
            ...
            L.push(`        </span>`);
            if (cht.text)
                L.push(`        <span style="font-family:Poppins,sans-serif;${textStyle(cht)};line-height:1.5;">${e(cht.text)}</span>`);
            L.push(`      </li>`);
        }
        L.push(`    </ul>`);
    }
...
export function generateCbHtml(d) {
    const id = uid();
    const headingPre = n(d.headingPre, 'bmsHeadingPre');
    const desc = n(d.desc, 'bmsDesc');
    const ctaText = n(d.ctaText, 'bmsCta');
