import type { TeamCard } from '../../types/cms';
...
export function generateTeamHtml(cards: TeamCard[]): string {
  if (!cards.length) return '<!-- No team members -->';
  ...
  for (const card of cards) {
    const eyebrow     = normalize(card.eyebrow     as any, 'teamEyebrow');
    const name        = normalize(card.name        as any, 'teamName');
    const designation = normalize(card.designation as any, 'teamDesignation');
    ...
    for (const f of validFeats) {
      const fv = normalize(f.value as any, 'teamFeatureValue');
      const fl = normalize(f.label as any, 'teamFeatureLabel');
      L.push(`      <div class="${uid}-feat">`);
      L.push(`        <div class="${uid}-fv" style="${textStyle(fv)}">${e(fv.text)}</div>`);
      if (fl.text) L.push(`        <div class="${uid}-fl" style="${textStyle(fl)}">${e(fl.text)}</div>`);
      L.push(`      </div>`);
    }
    L.push(`    </div>`);
    ...
    if (eyebrow.text) L.push(`      <div class="${uid}-ey" style="${textStyle(eyebrow)}">${e(eyebrow.text)}</div>`);
    L.push(`      <h3 class="${uid}-name" style="${textStyle(name)}">${e(name.text)}</h3>`);
    if (designation.text) L.push(`      <p class="${uid}-role" style="${textStyle(designation)}">${e(designation.text)}</p>`);
    for (const p of validParas) {
      const para = normalize(p as any, 'team');
      L.push(`      <p class="${uid}-para" style="${textStyle(para)}">${e(para.text)}</p>`);
    }
    if (validTags.length) {
      L.push(`      <div class="${uid}-tags">`);
      for (const t of validTags) {
        const tag = normalize(t as any, 'teamTag');
