import type { CfState } from '../../types/cms';
import { normalize, textStyle, escapeHtml } from '../../utils/text';

export function generateCfHtml(d: CfState): string {
  ...
  let leftHtml = '';
  if (lbl.text)     leftHtml += `<p style="font-family:'Poppins',sans-serif;margin:0 0 8px;text-transform:uppercase;${textStyle(lbl)}">${escapeHtml(lbl.text)}</p>`;
  if (company.text) leftHtml += `<h3 style="font-family:'Poppins',sans-serif;margin:0 0 6px;${textStyle(company)}">${escapeHtml(company.text)}</h3>`;
  if (addr.text)    leftHtml += `<p style="font-family:'Poppins',sans-serif;margin:0;line-height:1.5;${textStyle(addr)}">${addr.text}</p>`;
