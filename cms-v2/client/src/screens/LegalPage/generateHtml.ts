import type { LegalPageState, PolicyBlock } from '../../types/cms';
...
function polGridHtml(cols: number | string, gap: string, itemsHtml: string): string {
...
function polBuildBlock(b: PolicyBlock, accent: string): string {
  if (!b || !b.type) return '';
  ...
  if (b.type === 'bullets') {
    ...
    return '<ul style="margin:0 0 16px;padding:0;list-style:none;">'
      + items.map(t =>
          `<li style="display:flex;align-items:flex-start;gap:10px;margin-bottom:8px;font-family:'Poppins',sans-serif;font-size:15px;color:#374151;line-height:1.6;">`
  ...
  if (b.type === 'cards') {
    const items = (b.items || []).filter((c: any) => c.label || c.text);
    if (!items.length) return '';
    const cardHtml = items.map((c: any) =>
  ...
  if (b.type === 'rights') {
    const items = (b.items || []).filter((r: any) => r.title || r.text);
    if (!items.length) return '';
    const rightHtml = items.map((r: any) =>
  ...
  if (b.type === 'definitions') {
    const items = (b.items || []).filter((d: any) => d.term || d.desc);
    if (!items.length) return '';
    return '<ul style="margin:0 0 16px;padding:0;list-style:none;">'
      + items.map((d: any) =>
          `<li style="display:flex;align-items:flex-start;gap:10px;margin-bottom:10px;font-family:'Poppins',sans-serif;font-size:15px;color:#374151;line-height:1.6;">`
          + `<span style="color:${xA(acc)};font-size:18px;line-height:1.4;flex-shrink:0;margin-top:1px;">&#8226;</span>`
          + `<span><strong style="color:#1a1a1a;">${xH(d.term)}</strong>${d.desc ? ' &#8212; ' + xH(d.desc) : ''}</span></li>`
        ).join('')
      + '</ul>';
  }
  ...
  if (b.type === 'icon-cards') {
    const items = (b.items || []).filter((c: any) => c.title || c.desc);
    if (!items.length) return '';
    const icHtml = items.map((c: any) => {
  ...
  if (b.type === 'link-cards') {
    const items = (b.items || []).filter((c: any) => c.title || c.desc);
    if (!items.length) return '';
    const lcHtml = items.map((c: any) =>
      `<div style="border:1px solid #e5e7eb;border-radius:10px;padding:16px 18px;display:flex;align-items:flex-start;gap:14px;">`
...
export function generateLegalPageHtml(data: LegalPageState): string {
  ...
  const metaDots = (data.meta || []).map(m =>
    `<span style="font-family:'Poppins',sans-serif;font-size:13px;color:#94a3b8;">&#8226; ${xH(m)}</span>`
  ).join(' ');
