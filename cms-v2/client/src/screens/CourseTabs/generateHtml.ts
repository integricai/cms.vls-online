import type { CourseTabsState, CourseTabBlock, CourseTabCard, CourseTabStep, CourseTabSupportRow } from '../../types/cms';
...
function safeUrl(v: string | undefined): string {
  const s = (v ?? '').trim();
  if (!s) return '#';
  if (/^(https?:\/\/|\/|#|mailto:)/.test(s)) return escapeHtml(s);
  return '#';
}

function renderCard(card: CourseTabCard, style: 'inc' | 'support' | 'more'): string {
...
function renderStep(step: CourseTabStep, index: number, isLast: boolean): string {
...
function renderBlock(blk: CourseTabBlock, uid: string): string {
  const d = blk.data;
...
export function generateCourseTabsHtml(d: CourseTabsState): string {
  const uid  = 'vct' + Date.now().toString(36);
  const tabs = d.tabs ?? [];

  function makeClick(panelId: string): string {
