...
export function generatePageDescWithMenuHtml(d) {
    ...
    const navLinks = (d.menuItems || []).map(item => `<a class="${uid}-navlink" href="javascript:void(0)" onclick="${uid}nav(${JSON.stringify(item.scrollTarget)})">${escapeHtml(item.title)}</a>`).join('\n');
