...
@media(max-width:640px){
  .vls-ev-body{display:flex;flex-direction:column;padding:16px 18px;}
  .vls-ev-top{display:contents;}
  .vls-ev-copy{order:1;}
  .vls-ev-desc{order:2;margin-top:14px;}
  .vls-ev-cta-wrap{order:3;width:100%;margin-top:14px;justify-content:stretch;}
  .vls-ev-cta{width:100%;text-align:center;}
}
</style>`;
}
function eventCardCssRules() {
    return [
        '.vls-ev-card{font-family:Poppins,sans-serif;display:flex;border-radius:12px;border:1px solid #e0e0f0;overflow:hidden;max-width:100%;background:#fff;box-shadow:0 2px 12px rgba(0,0,0,.06);}',
        '.vls-ev-date{background:#534AB7;width:76px;flex-shrink:0;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:2px;padding:16px 8px;}',
        '.vls-ev-body{flex:1;min-width:0;padding:16px 20px;}',
...
export function renderEventCardHtml(event) {
    ...
    return `${eventCardCss()}<div class="vls-ev-card">
  <div class="vls-ev-date">
  ...
  <div class="vls-ev-body">
    <div class="vls-ev-top">
      <div class="vls-ev-copy">
      ...
      ${cta.text ? `<div class="vls-ev-cta-wrap"><a class="vls-ev-cta" href="${attr(event.ctaUrl || '#')}" style="${textStyle(cta)}">${escapeHtml(cta.text)}</a></div>` : ''}
    </div>
    ${event.description?.length ? `<div class="vls-ev-desc"><hr style="border:none;border-top:1px solid #e5e7eb;margin:0 0 14px;">${descHtml(event.description)}</div>` : ''}
  </div>
</div>`;
}
export function renderEventsListPreview(events) {
    const now = new Date();
...
export function buildEventEmbedCode(eventId, publicUrl) {
...
export function buildEventsListEmbedCode(publicUrl) {
