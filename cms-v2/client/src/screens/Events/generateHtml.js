...
export function renderEventCardHtml(event) {
  ...
  <div style="flex:1;padding:16px 20px;position:relative;">
    ${cta.text ? `<a href="${attr(event.ctaUrl || '#')}" style="position:absolute;top:16px;right:16px;display:inline-block;padding:9px 18px;background:#534AB7;border-radius:6px;text-decoration:none;white-space:nowrap;${textStyle(cta)}">${escapeHtml(cta.text)}</a>` : ''}
    <h2 style="font-family:Poppins,sans-serif;margin:0 0 4px;text-align:left;${textStyle(name)}${cta.text ? 'padding-right:170px;' : ''}">${escapeHtml(name.text || 'Untitled')}</h2>
