...
export function renderEventCardHtml(event: VlsEvent): string {
  ...
  <div style="flex:1;min-width:0;padding:16px 20px;">
    <h2 style="font-family:Poppins,sans-serif;margin:0 0 6px;text-align:left;${textStyle(name)}">${escapeHtml(name.text || 'Untitled')}</h2>
    ${hosts.text ? `<div style="font-family:Poppins,sans-serif;margin-bottom:8px;text-align:left;${textStyle(hosts)}">Host: ${escapeHtml(hosts.text)}</div>` : ''}
    ${info ? `<div style="display:flex;flex-wrap:wrap;gap:16px;margin-bottom:12px;">${info}</div>` : ''}
    ${cta.text ? `<div style="margin-bottom:${event.description?.length ? '14px' : '0'};"><a href="${attr(event.ctaUrl || '#')}" style="display:inline-block;padding:9px 20px;background:#534AB7;border-radius:6px;text-decoration:none;white-space:nowrap;${textStyle(cta)}">${escapeHtml(cta.text)}</a></div>` : ''}
