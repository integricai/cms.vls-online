...
export function generateProgramCardsHtml(componentId, componentName, data) {
    const uid = 'vlspg' + Math.random().toString(36).slice(2, 7);
    const topics = data.topics || [];
    const allCards = topics.flatMap((t) => (t.cards || []).map((c) => ({
        topicId: t.id,
    ...
    const dataJson = JSON.stringify({
        sections: sectionsForHtml.map(s => ({ id: s.id, name: s.name, topics: (s.topics || []).map((t) => ({ id: t.id })) })),
