...
export function generateCourseHeroHtml(d) {
    ...
    if (validPills.length) {
        const pillItems = validPills.map(p => {
            let inner = '';
            if (p.icon)
                inner += `<span style="font-size:16px;line-height:1;">${escapeHtml(p.icon)}</span>`;
            if (p.value)
                inner += `<strong style="font-family:'Poppins',sans-serif;font-weight:700;color:${pillVc};font-size:13px;">${escapeHtml(p.value)}</strong>`;
            if (p.label)
                inner += `<span style="font-family:'Poppins',sans-serif;color:${pillLc};font-size:13px;">${escapeHtml(p.label)}</span>`;
