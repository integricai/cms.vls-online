...
const COURSE_FINDER_BANNER_RUNTIME = (0, fs_1.readFileSync)((0, path_1.join)(__dirname, '../assets/course-finder-banner.runtime.js'), 'utf8');
...
function setPublicJsonCache(res) {
...
async function fetchContentData(key) {
    try {
        const row = await (0, content_1.getContent)(key);
...
async function getPublicContentData(key) {
    const now = Date.now();
    ...
    if (existing?.inFlight) {
        try {
            ...
            if (existing.data !== undefined && existing.staleUntil > now)
                return existing.data;
            throw err;
        }
    }
...
router.get('/events', async (_req, res, next) => {
    try {
        const data = (await getPublicContentData('vls-events')) ?? {};
        allowPublicCors(res);
...
router.get('/blog', async (_req, res, next) => {
    try {
        const data = (await getPublicContentData('vls-blog-posts')) ?? {};
