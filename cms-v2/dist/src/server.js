...
const bookDiscountCodes_1 = __importDefault(require("./routes/bookDiscountCodes"));
...
app.use('/api/book-discount-codes', bookDiscountCodes_1.default);
...
app.get('/blog/:topic/:slug', async (req, res, next) => {
    try {
        ...
        res.setHeader('Content-Type', 'text/html; charset=utf-8');
        res.setHeader('Cache-Control', 'no-store');
        return res.send((0, blogRender_1.renderBlogArticle)(post, settings));
    }
    catch (err) {
        next(err);
    }
